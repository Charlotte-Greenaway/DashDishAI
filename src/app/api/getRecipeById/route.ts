...
import recipeModel from '@/models/recipeModel';
 ...
 export async function POST(request: NextRequest) {
    try {
        ...
        const missingIngsArr= missingIngs.split(",")
        console.log(missingIngsArr.length)
        if (docs) {
        ...
        console.log(error);
        return NextResponse.json({ message: "An error occurred while fetching recipe", status: 500 });
    }
}